    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/80 backdrop-blur-xs flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="p-5 space-y-4 bg-slate-50/50">
            ...
            <div className="flex items-center gap-2">
              ...
              {isLive && (
                <span className="text-xs font-semibold text-rose-600 animate-pulse flex items-center gap-1">
                  <Activity className="w-3.5 h-3.5" /> Trận đấu đang diễn ra
                </span>
              )}
            </div>
          ...
          {/* Winner Banner if match finished */}
          {isFinished && match.winnerId && (
            <div className="p-3 bg-gradient-to-r from-amber-500/10 via-amber-500/20 to-amber-500/10 border border-amber-300 rounded-xl flex items-center justify-center gap-2 text-center text-xs font-bold text-amber-900">
              <Trophy className="w-4 h-4 text-amber-600 fill-amber-500" />
              <span>
                CẶP ĐÔI CHIẾN THẮNG:{' '}
                {match.winnerId === match.pair1?.id ? (
                  <span className="text-slate-950 font-black">
                    {match.pair1?.player1?.name || 'VĐV 1'} &amp; {match.pair1?.player2?.name || 'VĐV 2'} ({match.pair1?.player1?.club || match.pair1?.club || 'ISC'})
                  </span>
                ) : (
                  <span className="text-slate-950 font-black">
                    {match.pair2?.player1?.name || 'VĐV 1'} &amp; {match.pair2?.player2?.name || 'VĐV 2'} ({match.pair2?.player1?.club || match.pair2?.club || 'ISC'})
                  </span>
                )}
              </span>
            </div>
          ...
          <div className="space-y-3">
            ...
            >
              <div className="flex items-center justify-between mb-2 pb-1.5 border-b border-slate-100">
                <div className="flex items-center gap-2">
                  <span className="text-[11px] font-bold bg-[#0F172A] text-white px-2 py-0.5 rounded">
                    CẶP 1
                  </span>
                </div>
                {isPair1Winner && (
                  <span className="inline-flex items-center gap-1 text-xs font-black text-amber-800 bg-amber-100 px-2 py-0.5 rounded-full border border-amber-300">
                    <Award className="w-3.5 h-3.5 text-amber-600 fill-amber-500" /> THẮNG TRẬN
                  </span>
                )}
                {isLive && match.currentServingPairId === match.pair1?.id && (
                  <span className="inline-flex items-center gap-1 text-[11px] font-bold text-rose-600 bg-rose-50 px-2 py-0.5 rounded-full border border-rose-200">
                    <Activity className="w-3 h-3 animate-pulse" /> Đang giao cầu
                  </span>
                )}
              </div>
            ...
            <div className="bg-[#0F172A] text-white rounded-xl p-3.5 text-center">
              ...
              {isBestOf3 ? (
                <div className="space-y-2">
                  <div className="flex items-center justify-center gap-4 text-3xl sm:text-4xl font-extrabold tabular-nums tracking-tight">
                  ...
                  <div className="flex items-center justify-center gap-2 pt-2 border-t border-slate-800 flex-wrap">
                    {match.sets.map((set, idx) => (
                      <div
                        key={idx}
                        className="bg-slate-800/80 px-2.5 py-1 rounded text-xs font-mono"
                      >
                        <span className="text-slate-400 mr-1">Set {idx + 1}:</span>
                        <span
                          className={
                ...
                <div className="flex items-center justify-center gap-5 text-3xl sm:text-4xl font-black tabular-nums">
                  <span
                    className={
                      isPair1Winner
                        ? 'text-amber-400'
        ...
        {/* Footer */}
        <div className="bg-white px-5 py-3.5 border-t border-slate-200 flex items-center justify-end">
          <button
            onClick={onClose}
