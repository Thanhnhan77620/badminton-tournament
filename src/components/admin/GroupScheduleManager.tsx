      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <span className="text-[10px] text-slate-500">
              Tạo tay, tùy biến hoặc import danh sách thứ tự trận
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 self-start sm:self-auto">
          ...
          <button
            ...
            disabled={isReadOnly || groupPairs.length < 2}
      ...
      {activeTab === 'list' && (
        <div className="space-y-2.5">
          {groupMatches.length === 0 ? (
            <div className="py-6 px-4 text-center border-2 border-dashed border-slate-200 rounded-xl space-y-2 bg-slate-50/50">
              <Calendar className="w-7 h-7 text-slate-300 mx-auto" />
              <p className="text-xs font-bold text-slate-600">Chưa có trận đấu nào cho Bảng {group}</p>
              <p className="text-[11px] text-slate-400 max-w-sm mx-auto">
                Bạn có thể bấm <strong>&quot;Tạo Tay&quot;</strong> để tạo từng trận hoặc bấm <strong>&quot;Import Thứ Tự&quot;</strong> để nhập format <code className="bg-slate-200 px-1 py-0.5 rounded text-slate-800">1, A1, A2 | 2, A3, A5</code>.
              </p>
            </div>
          ) : (
            <div className="space-y-2">
              {/* Drag & Drop Hint Banner */}
              <div className="flex items-center justify-between px-2.5 py-1.5 bg-indigo-50/80 rounded-lg border border-indigo-100 text-[11px] text-indigo-700">
                <span className="flex items-center gap-1.5 font-medium">
                  <MoveVertical className="w-3.5 h-3.5 text-indigo-600 shrink-0" />
                  Kéo thả (Drap/Drop) thẻ hoặc bấm mũi tên <span className="font-bold">↑ ↓</span> để đổi thứ tự trận
                </span>
                <span className="text-[10px] font-bold bg-indigo-100/80 px-1.5 py-0.5 rounded text-indigo-800">
      ...
      <div className="mt-4 pt-3 border-t border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-bold text-slate-700">
            Trạng thái Public Lịch Bảng {group}:{' '}
            <strong className={isPublished ? 'text-emerald-700' : 'text-amber-700'}>
              {isPublished ? 'Đã Công Khai' : 'Bản Nháp (Chưa Công Khai)'}
            </strong>
          </span>
        </div>
        ...
        <div className="flex items-center gap-2">
          {groupMatches.length > 0 && (
            <button
              ...
              disabled={isReadOnly}
              className="px-2.5 py-1.5 rounded-lg border border-rose-200 text-rose-700 hover:bg-rose-50 text-xs font-bold transition-colors cursor-pointer flex items-center gap-1"
