  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-6 space-y-6">
      ...
      <div className="space-y-3.5 bg-slate-50 p-4 rounded-2xl border border-slate-200">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-10 py-2.5 bg-white border border-slate-200 rounded-xl text-xs sm:text-sm font-medium text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-xs"
          />
          {searchQuery && (
            <button
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3 pt-1 border-t border-slate-200/70">
          {/* Group Filter Tabs */}
      ...
      {filteredMatches.length === 0 ? (
        <div className="p-12 text-center bg-slate-50/80 rounded-2xl border border-dashed border-slate-300 space-y-3">
          <div className="w-12 h-12 rounded-full bg-slate-200 flex items-center justify-center mx-auto text-slate-500">
            <Search className="w-6 h-6" />
          </div>
          <h4 className="text-base font-bold text-slate-800">Không tìm thấy trận đấu nào phù hợp</h4>
          <p className="text-xs text-slate-500 max-w-md mx-auto">
            Không có trận đấu nào khớp với từ khóa tìm kiếm &ldquo;{searchQuery}&rdquo; hoặc bộ lọc hiện tại.
          </p>
          <button
            onClick={() => {
              setSearchQuery('');
          ...
          >
            ...
            <span>Xem toàn bộ 24 trận đấu</span>
          </button>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredMatches.map(match => {
            ...
            const isBlocked = !readiness.ready && !isEmergencyUnlocked;
            ...
            return (
              <div
                ...
                className={`p-4 rounded-2xl border transition-all flex flex-col justify-between ${
                  isLive
                    ? 'bg-rose-50/50 border-rose-400 ring-2 ring-rose-400/30 shadow-md'
                    : isFinished
              ...
              >
                ...
                <div className="flex items-center justify-between pb-2.5 mb-2.5 border-b border-slate-100 text-xs">
                  <div className="flex items-center gap-2">
                    <span className="px-2.5 py-1 rounded-lg bg-slate-900 text-white font-extrabold text-xs">
                      Trận #{match.matchNumber}
                    </span>
                    <span className="font-bold text-slate-800">{match.roundLabel || match.court}</span>
                    <span className="text-slate-400 font-medium">&bull; {match.court}</span>
                  </div>

                  {/* PROMINENT STATUS BADGES */}
                  {isLive ? (
                    <span className="px-3 py-1 rounded-full bg-rose-600 text-white font-black text-xs shadow-xs animate-pulse flex items-center gap-1.5">
                      <span className="w-2 h-2 rounded-full bg-white animate-ping"></span>
                      🔴 ĐANG ĐẤU (LIVE)
                    </span>
                  ) : isFinished ? (
                    <span className="px-3 py-1 rounded-full bg-emerald-100 border border-emerald-300 text-emerald-800 font-extrabold text-xs flex items-center gap-1">
                      <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" />
                      ĐÃ HOÀN TẤT
                    </span>
                  ) : isBlocked ? (
                    <span className="px-3 py-1 rounded-full bg-amber-100 border border-amber-300 text-amber-900 font-extrabold text-xs flex items-center gap-1" title={readiness.reason}>
                      <Hourglass className="w-3.5 h-3.5 text-amber-700" />
                      CHỜ VÒNG TRƯỚC
                    </span>
                  ) : (
                    <span className="px-3 py-1 rounded-full bg-blue-100 border border-blue-300 text-blue-800 font-extrabold text-xs flex items-center gap-1">
                ...
                <div className="space-y-2 py-1">
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      <span
                        className={`text-sm sm:text-base px-2.5 py-1 rounded-lg font-black min-w-[34px] text-center tabular-nums ${
                          isPair1Winner
                      ...
                      {s2 && isFinished && (
                        <span className="text-xs px-2 py-1 rounded-lg bg-slate-100 font-bold text-slate-600 tabular-nums">
                          {s2.pair1Score}
                        </span>
                      )}
                      {s3 && isFinished && (
                ...
                <div className="mt-3 pt-2.5 border-t border-slate-100 flex items-center justify-between gap-2">
                  ...
                  <button
                    ...
                    disabled={isReadOnly}
                    className={`px-4 py-2 rounded-xl text-xs font-extrabold shadow-xs transition-all flex items-center gap-1.5 ${
                      isReadOnly
