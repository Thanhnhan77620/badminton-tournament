...
export const AdminPortal: React.FC = () => {
  ...
  const handleUnlockSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (['unlock', 'btc', 'admin', 'isc2026'].includes(unlockCode.trim().toLowerCase())) {
      toggleEmergencyUnlock(true);
      setShowUnlockModal(false);
      setUnlockCode('');
      setUnlockError('');
    } else {
      setUnlockError('Mã mở khóa khẩn cấp không chính xác! Vui lòng liên hệ Trưởng BTC.');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-100/70 text-slate-900 flex flex-col font-sans">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto py-6 px-4 sm:px-6 space-y-6">
        ...
        <div
          className={`rounded-2xl p-4.5 border transition-all shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-4 ${
            ...
            <div className="space-y-0.5">
              <div className="flex flex-wrap items-center gap-2">
                ...
                >
                  {tournament.status === 'IN_PROGRESS'
                    ? isEmergencyUnlocked
                      ? 'CHẾ ĐỘ MỞ KHÓA KHẨN CẤP'
                      : 'TRẠNG THÁI: ĐANG DIỄN RA (LOCKED)'
                    : tournament.status === 'COMPLETED'
                    ? 'TRẠNG THÁI: ĐÃ BẾ MẠC (READ ONLY)'
                    : 'TRẠNG THÁI: CHUẨN BỊ (DRAFT / UPCOMING)'}
                </span>
                <span className="text-xs font-extrabold">
                  {tournament.status === 'IN_PROGRESS'
                    ? isEmergencyUnlocked
                      ? 'Đang cho phép can thiệp danh sách & cặp đấu đặc cách'
                      : 'Đang bảo vệ dữ liệu giải đấu: Khóa xóa VĐV, Khóa tạo lại bảng & Khóa đổi cặp'
                    : tournament.status === 'COMPLETED'
                    ? 'Giải đấu đã kết thúc và lưu trữ vĩnh viễn'
                    : 'Giai đoạn thiết lập danh sách, bốc thăm & xếp lịch thi đấu'}
                </span>
              </div>
              <p className="text-xs opacity-80 leading-relaxed">
                {tournament.status === 'IN_PROGRESS'
                  ? isEmergencyUnlocked
                    ? '⚠️ Cảnh báo: Bạn đang ở chế độ khẩn cấp. Mọi thao tác xóa cặp đấu có thể ảnh hưởng đến lịch 24 trận đấu.'
                    : '✓ Chỉ cho phép: Cập nhật Tên Đơn vị VĐV, Số điện thoại và Nhập điểm bàn Trọng tài.'
                  : tournament.status === 'COMPLETED'
                  ? 'Tất cả các trận đấu và xếp hạng đã được đóng lại.'
                  : 'Sau khi hoàn tất chia bảng và lịch đấu, chuyển sang "Đang diễn ra" để khóa an toàn.'}
              </p>
            </div>
          </div>
              ...
              >
                {isEmergencyUnlocked ? (
                  <>
                    ...
                    <span>Đóng Khóa An Toàn</span>
                  </>
                ) : (
                  <>
                    <Unlock className="w-3.5 h-3.5 text-amber-600" />
                    <span>Mở Khóa Khẩn Cấp</span>
                  </>
                )}
              </button>
            )}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400">
                {activeSection === 'dashboard' && 'Tổng quan giải đấu, thông số nhanh và liên kết điều hành'}
                {activeSection === 'tournament' && 'Chỉnh sửa thông tin địa điểm, thời gian và banner giải'}
                {activeSection === 'rules' && 'Cập nhật điều lệ thi đấu, quy định tính điểm và giải thưởng'}
                {activeSection === 'players' && 'Quản lý 20 Vận Động Viên, phân nhóm hạt giống và import Excel/CSV'}
                {activeSection === 'groups' && 'Ghép 10 cặp đấu, bốc thăm phân Bảng A/B và sinh lịch 24 trận'}
                {activeSection === 'results' && 'Bàn trọng tài ghi điểm trực tiếp 24 trận và xử lý Walkover'}
              </p>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl border border-slate-200">
            ...
            <p className="text-xs text-slate-600 leading-relaxed">
              Giải đấu đang ở trạng thái <strong>ĐANG DIỄN RA</strong>. Để can thiệp sửa đổi đặc cách (ví dụ VĐV chấn thương nặng cần thay thế trước giờ thi đấu), vui lòng nhập mã xác thực BTC:
            </p>

            <form onSubmit={handleUnlockSubmit} className="space-y-4">
              <div>
                <label className="block text-[11px] font-bold uppercase text-slate-600 mb-1">
                  Mã Xác Thực Mở Khóa Khẩn Cấp (Gợi ý: <code className="text-blue-600">btc</code> hoặc <code className="text-blue-600">unlock</code>)
                </label>
                <input
                  type="password"
                  value={unlockCode}
                  onChange={e => setUnlockCode(e.target.value)}
                  placeholder="Nhập mã BTC..."
                  className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold focus:bg-white focus:outline-none focus:ring-2 focus:ring-amber-500"
                  autoFocus
                  required
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-100">
                ...
                <button
                  type="submit"
                  className="px-4 py-2 rounded-xl bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold shadow-md shadow-amber-600/30 cursor-pointer"
                >
                  Xác Nhận Mở Khóa
                </button>
              </div>
