...
Phan Tấn Hưng, ISC, Phạm Viết Thiện, ISC`;
  ...
  const parsedImportItems = useMemo(() => {
    ...
    return lines.map((line, idx) => {
      ...
      let prospectiveGroup: 'A' | 'B' = 'A';
      if (importTargetGroup === 'A') {
        prospectiveGroup = 'A';
      } else if (importTargetGroup === 'B') {
        prospectiveGroup = 'B';
      } else {
        const total = lines.length;
        const half = Math.ceil(total / 2);
        prospectiveGroup = idx < half ? 'A' : 'B';
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            {isLiveLocked
              ? 'Giải đang diễn ra: Đã khóa chia bảng, bốc thăm & tạo lại lịch để bảo vệ 24 trận đấu.'
              : isCompletedLocked
              ? 'Giải đã bế mạc: Cặp đấu & Bảng đấu ở chế độ chỉ đọc.'
              : 'Quy tắc: Trong một bảng đấu không được có cặp trùng. Ghép 2 VĐV thành 1 cặp đôi, xếp vào Bảng A / Bảng B và tạo lịch thi đấu tự động.'}
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <button
            ...
            title={isLiveLocked ? 'Đã khóa import khi giải đang diễn ra' : 'Import danh sách cặp đấu'}
          >
          ...
          <button
            ...
            title={isLiveLocked ? 'Đã khóa ghép thêm cặp khi giải đang diễn ra' : 'Ghép cặp mới'}
          >
      ...
      {/* Live locked notice in pairs & groups */}
      {isLiveLocked && (
        <div className="p-3.5 bg-amber-50 rounded-2xl border border-amber-200 text-amber-900 text-xs flex items-center justify-between gap-3 shadow-xs">
          <div className="flex items-center gap-2.5">
            <AlertTriangle className="w-4 h-4 text-amber-600 shrink-0" />
            <span>
              <strong>Bảo Vệ Cấu Trúc Bảng Đấu:</strong> Khi giải đấu ở trạng thái <strong>ĐANG DIỄN RA</strong>, chức năng xóa cặp, đổi bảng và sinh lại lịch trận đấu bị khóa để tránh làm sai lệch tỷ số và kết quả các trận đang đấu.
            </span>
          </div>
        </div>
      )}
      ...
      {/* Toast notifications */}
      {isSuccessMsg && (
        <div className="p-3.5 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs font-bold flex items-center justify-between gap-2 animate-in fade-in shadow-xs">
      ...
      {isErrorMsg && (
        <div className="p-3.5 rounded-xl bg-rose-50 border border-rose-200 text-rose-800 text-xs font-bold flex items-center justify-between gap-2 animate-in fade-in shadow-xs">
          <div className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-5 space-y-4">
          ...
          {pairsA.length === 0 ? (
            <div className="py-8 px-4 text-center border-2 border-dashed border-slate-200 rounded-xl space-y-2">
              ...
              <p className="text-xs font-bold text-slate-600">Chưa có cặp đấu nào trong Bảng A</p>
              <p className="text-[11px] text-slate-400">
                Nhấn &quot;Ghép Cặp Mới&quot; để thêm cặp đấu hoặc chuyển cặp từ Bảng B sang.
              </p>
            </div>
          ) : (
            <div className="space-y-2.5">
              {pairsA.map((pair, idx) => (
                <div
                  key={pair.id}
                  className="p-3 rounded-xl border border-slate-200 bg-slate-50/50 hover:bg-blue-50/30 transition-all flex items-center justify-between gap-3 group"
                >
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <button
                      ...
                      title={isLiveLocked ? 'Đã khóa đổi bảng khi giải đang diễn ra' : 'Chuyển sang Bảng B'}
                    >
                    ...
                    <button
                      ...
                      title={isLiveLocked ? 'Đã khóa sửa cặp khi giải đang diễn ra' : 'Sửa cặp đấu'}
                    >
                    ...
                    <button
                      ...
                      title={isLiveLocked ? 'Đã khóa xóa cặp khi giải đang diễn ra' : 'Xóa cặp đấu khỏi bảng'}
                    >
          ...
          <div className="pt-3 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-slate-700">
                Public Danh Sách Bảng A:{' '}
                <strong className={(tournament.isGroupAPublished ?? false) ? 'text-emerald-700' : 'text-amber-700'}>
                  {(tournament.isGroupAPublished ?? false) ? 'Đã Công Khai' : 'Bản Nháp (Chưa Công Khai)'}
                </strong>
              </span>
            </div>
            ...
            <button
              ...
              className={`px-3.5 py-1.5 rounded-xl text-xs font-bold shadow-xs transition-all cursor-pointer flex items-center gap-1.5 self-start sm:self-auto ${
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-5 space-y-4">
          <div className="flex items-center justify-between pb-3 border-b border-slate-100">
            <div className="flex items-center gap-2">
              <span className="w-7 h-7 rounded-xl bg-amber-500 text-slate-950 text-xs font-black flex items-center justify-center shadow-xs">
                B
              </span>
              <h3 className="text-sm font-extrabold text-slate-900">
          ...
          {pairsB.length === 0 ? (
            <div className="py-8 px-4 text-center border-2 border-dashed border-slate-200 rounded-xl space-y-2">
              <Users className="w-8 h-8 text-slate-300 mx-auto" />
              <p className="text-xs font-bold text-slate-600">Chưa có cặp đấu nào trong Bảng B</p>
              <p className="text-[11px] text-slate-400">
                Nhấn &quot;Ghép Cặp Mới&quot; để thêm cặp đấu hoặc chuyển cặp từ Bảng A sang.
              </p>
            </div>
          ) : (
            <div className="space-y-2.5">
              {pairsB.map((pair, idx) => (
                <div
                  key={pair.id}
                  className="p-3 rounded-xl border border-slate-200 bg-slate-50/50 hover:bg-amber-50/30 transition-all flex items-center justify-between gap-3 group"
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <span className="w-6 h-6 rounded-lg bg-amber-100 text-amber-900 text-[11px] font-black flex items-center justify-center shrink-0">
                      {pair.code || `B${idx + 1}`}
                    </span>
                    <div className="min-w-0">
                      <p className="text-xs font-bold text-slate-900 truncate">{pair.name}</p>
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <button
                      ...
                      title={isLiveLocked ? 'Đã khóa đổi bảng khi giải đang diễn ra' : 'Chuyển sang Bảng A'}
                    >
                      <ArrowRightLeft className="w-3 h-3" />
                      <span>Sang A</span>
                    </button>
          ...
          <div className="pt-3 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
            <div className="flex items-center gap-2">
              ...
              />
              <span className="text-xs font-bold text-slate-700">
                Public Danh Sách Bảng B:{' '}
                <strong className={(tournament.isGroupBPublished ?? false) ? 'text-emerald-700' : 'text-amber-700'}>
                  {(tournament.isGroupBPublished ?? false) ? 'Đã Công Khai' : 'Bản Nháp (Chưa Công Khai)'}
                </strong>
              </span>
            </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                <div>
                  ...
                  <select
                    ...
                    onChange={e => setImportTargetGroup(e.target.value as 'AUTO' | 'A' | 'B')}
                  ...
                  >
                    <option value="AUTO">Tự động chia đều Bảng A &amp; B</option>
